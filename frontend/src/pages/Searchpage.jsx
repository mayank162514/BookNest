...
const Searchpage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // No need to await searchBooks here, the useEffect will handle it when the URL changes
  ...
  useEffect(() => {
    ...
    // The dependency array is now correct. This effect will re-run
    // anytime the URL search string changes.
  }, [location.search]); // Corrected dependency
  ...
  return (
    <div className='min-h-screen text-[#252422] bg-[#f5f5f5] px-4 md:px-12 pb-10'>
      <p
        ...
        // Using navigate(-1) provides a more natural "back" behavior
        onClick={() => navigate(-1)}
      >
      ...
      {books.length > 0 ? (
        <div className='flex flex-wrap justify-center gap-5 lg:gap-8 w-full max-w-6xl mx-auto'>
          {books.map((book) => ( // Removed index from parameters
            // Corrected Key: Use the unique book ID
      ...
      ) : <p className='text-center text-gray-500'>No books found. Try a different search term.</p>}
    </div>
  );
};
